...
```bash
curl -fsSL https://raw.githubusercontent.com/pervezfunctor/niri-config/main/setup > setup
./setup
rm setup
```
...
For installing on a different system, change references to current repository in `setup` and follow the same instructions.
...
| `setup` | Initial setup script (interactive) |
| `vm` | VM testing script |
